...
```
"ข้อนี้ต้องรับสีจาก input สองตัว จากนั้นใช้ condition ในการตรวจว่า input ทั้งสองตัวนั้นเป้นสีอะไรเเละทำการจับคู่ว่าถ้าสีนี้มาผสมกับสีนี้จะได้สีอะไรเเละจัดการ output ออกมาทีละตัว เเต่หาก input ไม่ตรงกับสีที่ตั้งไว้ ให้ output ออกมาเป็น Error "
```
...
Step 3:ทำการลองเขียนโค้ดเเละหาข้อมูลจากสไลด์ Prepro
Step 4:ลองใช้ nested condition ในการทำโจทย์ข้อนี้
Step 5:ลองส่งใน ijudge หาผิดให้ทำการเเก้ไขหรือให้เพื่อนที่ทำเสร็จเเล้วสอนเรา
```
...
```
ไม่เหมือน เพราะตอนเเรกที่ทำส่งได้ใช้ nested condition เเต่ไม่ผ่าน PEP-8 เลยไปถามเพื่อน จึงเปลี่ยนไปใช้ elif เเทน 
```
...
```
็เป็นการเช็คว่าผสมสีถูกต้องมั้ย
```
...
```
เผื่อเช็คว่าหากสีเหมือนกันจะเป็นอะไร
```
...
```
เผื่อตรวจว่าหากสีไม่ตรงกับที่ตั้งไว้ จะเป็นยังไง
```
...
```
ช่วยตรวจสอบว่าผิดPEP-8 ตรงไหนเเละควรใช้วิธีไหนที่จะถูกต้อง
```

คุณยังทำอะไรด้วยตนเอง

```
เริ่มเขียนโค้ดในเเบบที่คิดไว้โดยใช้ nested condition เเต่ไม่ผ่าน
```
