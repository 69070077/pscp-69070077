...
```text
input รับข้อความมาเป็น string
output เเสดงสระที่มีเเละจำนวนของสระที่มี
constraints  ต้องเเสดงเฉพาะตัวสระที่มีในข้อความเท่านั้น
```
...
```text
รับข้อความ เเละทำเป็นตัวเล็ก
ใช้ condition + .count() > 0 เพื่อเช็ค ว่ามีสระเหล่านี้มั้ย
หากมีเเสดงผลโดยใช้ format string + .count()
...
```text
บอกวิธีที่จะให้โค้ดง่ายขึ้น
```

คุณยังทำอะไรด้วยตนเอง

```text
การคิดโลจิกเเละทำโค้ดเเบบเเรกด้วยตัวเอง
```
